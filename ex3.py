...
def objfun(param, x, y, gain=200):
    ...
    loss = relu(W[3])+relu(-W[1])#+relu(-W[2])#-sigmoid(param, 2, 10)*0.001+W[2]
    rmse_diff = calc_rmse(np.diff(y), np.diff(y_calc))
    fval = rmse + rmse_diff + loss*100
    return fval
...
# リッジ回帰における未知係数の計算
def calc_W(param, x, y, gain, alpha=0.01):
    Phi   = calc_Phi(x, param, gain)
...
def main():
    ...
    for i in range(N_trial):
        ...
        random.seed(i+1000)
        param_true = [random.randint(-1, 201)/100]
        random.seed(i+3000)
        param_true += [random.randint(100, 130)/100]
        gain = 2000
        e=0
        if N_trial/2<=i:
            e=1
        ...
        # パラメータの最適化
        l_param_init = np.linspace(0, 1.5, 21)
        fval=1e8
